...
test('splitWhenEvery/ single split', (t) => {
  ...
  t.isEquivalent(output, [[0, 1, 2], [3, 4, 5]])
})
...
test('splitWhenEvery/ multi split', (t) => {
  ...
  const output = splitWhenEvery((n1, n2) => (n2 ?? 0) - (n1 ?? 0) === 0)(input)
  t.isEquivalent(output, [[0, 1, 2], [3, 4], [5, 6]])
})
...
test('splitWhenEvery/ multi split 2', (t) => {
  ...
  const output = splitWhenEvery((n1, n2) => (n2 ?? 0) - (n1 ?? 0) === 0)(input)
  t.isEquivalent(output, [[0, 1, 2], [3, 4], [6]])
})
...
test.only('splitWhenEvery/ xxxx', (t) => {
  t.plan(1)
  const input: number[] = [
    1,  2,  3,  4,  7,  8,  9, 10, 11,
